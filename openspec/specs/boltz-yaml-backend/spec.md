...
Extend the Boltz platform backend to accept, validate, and store YAML configuration files alongside existing FASTA inputs. Support an optional `config_file` field in prediction requests and pass the YAML config as a `config_uri` parameter to the Vertex AI pipeline runner.
